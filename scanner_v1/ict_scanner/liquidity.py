...
def session_levels(df: pd.DataFrame, session: str, now: pd.Timestamp | None = None) -> tuple[float | None, float | None]:
    ...
    """
    x = _with_dt(df)
    local = x._dt.dt.tz_convert(NY)
    x = x.assign(_ny=local)
    now_ny = (now if now is not None else x._dt.iloc[-1]).tz_convert(NY)
    d = now_ny.date()

    if session == "asia":
        start = pd.Timestamp.combine(d - timedelta(days=1), pd.Timestamp("20:00").time(), tzinfo=NY)
        end = pd.Timestamp.combine(d, pd.Timestamp("00:00").time(), tzinfo=NY)
    elif session == "london":
        start = pd.Timestamp.combine(d, pd.Timestamp("02:00").time(), tzinfo=NY)
        end = pd.Timestamp.combine(d, pd.Timestamp("05:00").time(), tzinfo=NY)
    elif session == "ny":
        start = pd.Timestamp.combine(d, pd.Timestamp("07:00").time(), tzinfo=NY)
        end = pd.Timestamp.combine(d, pd.Timestamp("10:00").time(), tzinfo=NY)
    else:
...
def nearest_draw(price: float, direction: str, m: LiquidityMap) -> tuple[str | None, float | None]:
    levels = m.to_dict()
    if direction == "LONG":
        valid = [(k, v) for k, v in levels.items() if v is not None and v > price and k.endswith("high") or (v is not None and v > price and k in {"pdh", "pwh"})]
        return min(valid, key=lambda kv: kv[1], default=(None, None))
    valid = [(k, v) for k, v in levels.items() if v is not None and v < price and (k.endswith("low") or k in {"pdl", "pwl"})]
